...
Buffet::~Buffet() {
    close();
    sem_destroy(&this->buffetState); // buffet mutex that prevents removing slices simultaneously
    sem_destroy(&this->vegMutex); // vegetarian mutex starts off allowing 1 vegetarian at a time
    sem_destroy(&this->omniMutex); // // vegetarian mutex starts off allowing 1 omnivore at a time
    sem_destroy(&this->addState);

    sem_destroy(&this->fullBlock);
    sem_destroy(&this->emptyBlock);
    sem_destroy(&this->omniBlock);
    sem_destroy(&this->vegBlock);
}
