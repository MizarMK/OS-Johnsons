...
void free(void *ptr) throw () {
    ...
    MemBlock *lastNode = nullptr, *rpredecessorNode = nullptr, *lpredecessorNode = nullptr, *ln, *rn;
    ...
    for (MemBlock **bptr = &free_list; *bptr; bptr = &((*bptr)->next)) {
        if ((MemBlock*)((char*)*bptr + (*bptr)->size) == block) {
            leftFree = true;
            lpredecessorNode = lastNode;
            ln = *bptr;
        }
        if ((MemBlock*)((char*)block + block->size) == *bptr) {
            rightFree = true;
    ...
    else if (rightFree and !leftFree) { // only works if the block being coalesced isn't the start of free_list
        //cout << "coalescing for right neighbor..." << endl;
        if (rpredecessorNode)
            rpredecessorNode->next = rn->next;
        block->next = free_list;
        free_list = block;
        if (rn == free_list->next) {
            free_list->next = rn->next;
        }
        block->size = block->size + rn->size;
    }
    else if (leftFree and rightFree) {
        //cout << "coalescing for both neighbors..." << endl;
        if (lpredecessorNode) {
            lpredecessorNode->next = rn->next;
            ln->next = free_list;
        }
        free_list = block - 1;
        ln->size += (block->size + rn->size);
    }
...
void memReport() {
    ...
    cout << "Used Bytes: " << allocatedBytes - fragTrack - (allocatedBlocks * 8) << endl;
    ...
    if (free_list) {
        ...
        if (bptr) {
            cout << bptr->size - 8 << endl;
            freeBlocks++;
            bytes += bptr->size;
        }
    }
    cout << "Blocks: " << freeBlocks << endl;
    cout << "Bytes: " << bytes - (freeBlocks * 8) << endl << endl;
}
